  ...
  async function createTodo(e: React.FormEvent) {
    ...
    if (newTodoContent.trim()) {
      await client.models.Todo.create({
        ...
        order: (todos.length + 1) * 1000, // Use a large interval for easy reordering
      });
      setNewTodoContent("");
    }
  }
    ...
    <main className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <ul className="space-y-3">
        {todos.map((todo, index) => (
          <li key={todo.id} className="flex items-center justify-between bg-gray-50 p-3 rounded-lg hover:shadow-md transition-shadow">
